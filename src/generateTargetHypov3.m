function [Xhypo] = generateTargetHypov3(Xpred,nbrOfMeas,nbrOfGlobHyp, Pd, H, R, Z, motionModel, nbrPosStates, nbrMeasStates)
 ...
 global color, global rescaleFact, global thresholdEst
...
for j = 1:nbrOfGlobHyp
    for i = 1:size(Xpred{j},2)
        ...
        if Xhypo{j,nbrOfMeas+1}(i).r > thresholdEst
        ...
        for i = 1:size(Xpred{j},2)
            if strcmp(motionModel,'cvBB')
                ...
                [Xhypo{j,z}(i).state, Xhypo{j,z}(i).P, Xhypo{j,z}(i).S(1:nbrMeasStates,1:nbrMeasStates),v]...
                    = KFUpd(Xpred{j}(i).state, H(1:nbrMeasStates,:),Xpred{j}(i).P, R(1:nbrMeasStates,1:nbrMeasStates), Z(1:nbrMeasStates,z));
            end
            % Enable if gating
            ...
            if(gatingv3(v,Xhypo{j,z}(i).S(1:nbrMeasStates,1:nbrMeasStates))) % 100
                if strcmp(motionModel,'cvBB')
                    % Bound box size
                    [Xhypo{j,z}(i).state, Xhypo{j,z}(i).P, Xhypo{j,z}(i).S(nbrMeasStates+1:4,nbrMeasStates+1:4)] = ...
                        KFUpd(Xhypo{j,z}(i).state, H(nbrMeasStates+1:end,1:end), Xhypo{j,z}(i).P, R(nbrMeasStates+1:end,nbrMeasStates+1:end), Z(3:4,z));

                    % Only yaw
                    Xhypo{j,z}(i).w = Xpred{j}(i).w + log(Xpred{j}(i).r*Pd) + ...
                        log_mvnpdf(Z(1:nbrMeasStates,z), Xpred{j}(i).state(1:2), Xhypo{j,z}(i).S(1:nbrMeasStates,1:nbrMeasStates));
                    if(color)
                ...
                Xhypo{j,z}(i).box = Xhypo{j,z}(i).state(5:6);
