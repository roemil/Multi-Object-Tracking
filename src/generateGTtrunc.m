...
function ZGT = generateGTtrunc(set,sequence,datapath,nbrOfStates)
% set = 'training';
% sequence = '0000';
...
%     pxCoords = [cx,cy];
%     ZGT{1}(:,1) = [pxCoords(1);pxCoords(2);bbsize(1);bbsize(2);GT{trackID}(ind)];
%     count = 1;
%     oldFrame = GT{1}(3)+1;
%     for i = 4 : size(GT{1},1)
%         frame = GT{1}(i)+1;
...
%     end
    ...
    % This is [px py d]
    oldFrame = GT{1}(ind)+1;
    ZGT{oldFrame}(:,1) = [pxCoords(1);pxCoords(2);bbsize(1);bbsize(2);GT{trackID}(ind)]; % cx
    count = 2;
