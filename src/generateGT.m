...
if(nbrOfStates == 4)
    ...
    oldFrame = GT{1}(ind)+1;
    ZGT{oldFrame}(:,1) = [pxCoords(1);pxCoords(2);bbsize(1);bbsize(2);GT{trackID}(ind);map(cell2mat(GT{3}(ind)))]; % cx
    ...
    for i = 4 : size(GT{1},1)
        ...
        if(frame == oldFrame && (GT{trackID}(i) ~= -1))
            ...
            ZGT{frame}(:,count+1) = [pxCoords(1);pxCoords(2);bbsize(1);bbsize(2);GT{trackID}(i);map(cell2mat(GT{3}(i)))]; % cx
            count = count + 1;
            oldFrame = frame;
        elseif(GT{trackID}(i) ~= -1)
            ...
            pxCoords = [cx,cy];
            bbsize = [GT{9}(i) - GT{7}(i),GT{10}(i)-GT{8}(i)];
            ZGT{frame}(:,1) = [pxCoords(1);pxCoords(2);bbsize(1);bbsize(2);GT{trackID}(i);map(cell2mat(GT{3}(i)))]; % cx
            count = 1;
            oldFrame = frame;  
        end
    end
...
%     pxCoords = [cx,cy];
%     bbsize = [GT{9}(ind) - GT{7}(ind),GT{10}(ind)-GT{8}(ind)];
%     ZGT{1}(:,1) = [pxCoords(1);pxCoords(2);bbsize(1);bbsize(2);GT{trackID}(ind)]; % cx
%     count = 2;
%     oldFrame = GT{1}(ind)+1;
%     for i = (ind+1) : size(GT{1},1)
%         frame = GT{1}(i)+1;
...
%             pxCoords = [cx,cy];
%             bbsize = [GT{9}(i) - GT{7}(i),GT{10}(i)-GT{8}(i)];
%             ZGT{frame}(:,count) = [pxCoords(1);pxCoords(2);bbsize(1);bbsize(2);GT{trackID}(i)]; % cx
%             count = count + 1;
%             oldFrame = frame;
%         elseif(GT{trackID}(i) ~= -1)
%             %R1(1:3,1:3) = Ry(i);
...
%     pxCoords = [cx,cy];
%     bbsize = [GT{9}(ind) - GT{7}(ind),GT{10}(ind)-GT{8}(ind)];
%     ZGT{1}(:,1) = [pxCoords(1);pxCoords(2);GT{zInd}(3);bbsize(1);bbsize(2);GT{trackID}(ind)]; % cx
%     count = 2;
%     oldFrame = GT{1}(ind)+1;
%     for i = (ind+1) : size(GT{1},1)
%         frame = GT{1}(i)+1;
...
%             pxCoords = [cx,cy];
%             bbsize = [GT{9}(i) - GT{7}(i),GT{10}(i)-GT{8}(i)];
%             ZGT{frame}(:,count) = [pxCoords(1);pxCoords(2);GT{zInd}(i);bbsize(1);bbsize(2);GT{trackID}(i)]; % cx
%             count = count + 1;
%             oldFrame = frame;
%         elseif(GT{trackID}(i) ~= -1)
%             %R1(1:3,1:3) = Ry(i);
