function [vx, vy, sigmaQ] = testFindQ
...
meanSeqCovX = zeros(1,21);
meanSeqCovY = zeros(1,21);
...
for s = 0:20
        ...
        for i = 1:size(oldLabels,1)
            ...
            if ~isempty(labelInd)
                vx{str2num(seq)+1}{oldLabels(i)+1} = [vx{str2num(seq)+1}{oldLabels(i)+1}, ...
                    (newStates(1,labelInd)-oldStates(1,i))/T];
                vy{str2num(seq)+1}{oldLabels(i)+1} = [vy{str2num(seq)+1}{oldLabels(i)+1}, ...
                    (newStates(2,labelInd)-oldStates(2,i))/T];
            end
        end
    ...
    covX = zeros(1,size(vx{str2num(seq)+1},2));
    covY = zeros(1,size(vy{str2num(seq)+1},2));
    ind = 1;
    for i = 1:size(vx{str2num(seq)+1},2)
        if ~isempty(vx{str2num(seq)+1}{i}) && size(vx{str2num(seq)+1}{i},2) >= 2
            covX(ind) = cov(vx{str2num(seq)+1}{i}(1:end-1)-vx{str2num(seq)+1}{i}(2:end));
            covY(ind) = cov(vy{str2num(seq)+1}{i}(1:end-1)-vy{str2num(seq)+1}{i}(2:end));
            ind = ind+1;
        end
    end
    meanSeqCovX(str2num(seq)+1) = mean(covX);
    meanSeqCovY(str2num(seq)+1) = mean(covY);
end

meanCovX = mean(meanSeqCovX);
meanCovY = mean(meanSeqCovY);
sigmaQ = mean([meanCovX,meanCovY]);

% vx{seq,target} instead of vx{seq}{target}
