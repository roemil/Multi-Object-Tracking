function [d, fp, fn, loc_err, car_err] = GOSPA23D(X,X3,Y,Y3,k,mode,c_thresh,GTdc)
...
if(xL > yL) % nonnegative symmetry
    ...
    d = d +0.5*c_thresh^p*(xL+yL-2*nbrAss-count_dontcare);
    d = d^(1/p);
    %car_err = (0.5*c_thresh^p*(xL+yL-2*(lAss)-count_dontcare))^1/p;
    car_err = (0.5*c_thresh^p*(xL+yL-2*nbrAss-count_dontcare))^1/p;
